...
double parseDurationSeconds(const JsonValue & json)
{
  if (!json.isObject()) {
    return 0.0;
  }
  const double sec = json["sec"].isNumber() ? json["sec"].asNumber() : 0.0;
  const double nsec = json["nanosec"].isNumber() ? json["nanosec"].asNumber() : 0.0;
  return sec + nsec * 1e-9;
}

ct::VehicleInfo finalizeVehicleInfo(ct::VehicleInfo info)
{
  info.vehicle_length_m = info.front_overhang + info.wheel_base + info.rear_overhang;
  info.vehicle_width_m = info.wheel_tread + info.left_overhang + info.right_overhang;
  info.max_longitudinal_offset_m = info.front_overhang + info.wheel_base;
  info.min_longitudinal_offset_m = -info.rear_overhang;
  return info;
}

ct::VehicleInfo loadVehicleInfoFromYaml(const std::string & path)
{
  ct::VehicleInfo info;
  info.wheel_base = 2.79;
  info.front_overhang = 1.0;
  info.rear_overhang = 1.1;
  info.left_overhang = 0.128;
  info.right_overhang = 0.128;
  info.wheel_tread = 1.64;
  std::ifstream ifs(path);
  if (!ifs) {
    return finalizeVehicleInfo(info);
  }
  ...
  while (std::getline(ifs, line)) {
    ...
    if (key == "wheel_base") info.wheel_base = number;
    else if (key == "front_overhang") info.front_overhang = number;
    else if (key == "rear_overhang") info.rear_overhang = number;
    else if (key == "left_overhang") info.left_overhang = number;
    else if (key == "right_overhang") info.right_overhang = number;
    else if (key == "wheel_tread") info.wheel_tread = number;
  }
  return finalizeVehicleInfo(info);
}
...
std::vector<ct::PredictedPath> parsePredictedPaths(const JsonValue & array)
{
  ...
  for (std::size_t i = 0; i < array.arraySize(); ++i) {
    ...
    } else if (element["time_step"].isObject()) {
      path.time_step = parseDurationSeconds(element["time_step"]);
    }
...
int runScenario1(
  const fs::path & scenario_dir, const bool scenario_include_goal_stop,
  const ct::VehicleInfo & vehicle_info)
{
  try {
    ...
    acp::ObstacleCruisePlannerNode node(info, behavior_param, vehicle_info);
...
int runScenario3(const fs::path & scenario_dir, const ct::VehicleInfo & vehicle_info)
{
  try {
    const fs::path kinematic_file = scenario_dir / "localization__kinematic_state.jsonl";
    const fs::path accel_file = scenario_dir / "localization__acceleration.jsonl";
...
void runDefaultMode(
  const std::map<std::string, std::string> & config, const ct::VehicleInfo & vehicle_info)
{
...
int main(int argc, char ** argv)
{
  ...
  std::string vehicle_info_path =
    "/home/sujin/Documents/01_GIT/autoware/src/universe/autoware.universe/common/"
    "autoware_test_utils/config/test_vehicle_info.param.yaml";

  for (int i = 1; i < argc; ++i) {
    const std::string arg = argv[i];
    if (arg == "--vehicle-info") {
      if (i + 1 < argc) {
        vehicle_info_path = argv[++i];
        continue;
      }
      std::cerr << "--vehicle-info requires a file path\n";
      return 1;
    }
  ...
  const auto vehicle_info = loadVehicleInfoFromYaml(vehicle_info_path);
  ...
  if (mode == RunMode::Scenario1) {
    ...
    return runScenario1(scenario_dir, scenario1_include_goal_stop, vehicle_info);
  }
  if (mode == RunMode::Scenario3) {
    ...
    return runScenario3(scenario_dir, vehicle_info);
  }
  ...
  runDefaultMode(config, vehicle_info);
  return 0;
}
